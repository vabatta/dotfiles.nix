---
source: https://github.com/cursor/plugins/blob/51a96e0dd838404da19ba83dc70aa21eef71f868/pstack/skills/architect/SKILL.md
name: architect
description: "Sketch types, signatures, and module structure before code, then stay in the loop while implementation fills in. Use when asked to architect or design something, or when non-trivial work would lock in the wrong shape."
---
...
Record one task-list entry per phase before starting when the host provides task
tracking. Otherwise keep the phase list in the working notes so autonomous runs
still show their position and do not silently skip phases.
...
Before sketching new interfaces, apply
[Use Before Build](../principle-use-before-build/SKILL.md). Inspect the standard
library, installed dependencies, and existing project APIs that could already
...
Run the **arena** workflow with the design-sketch task and the Phase A grounding artifacts. Pass `references/runner-prompt.md` as each runner's prompt when the host supports delegated candidates; otherwise produce the candidates sequentially. Each candidate produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it.

Use the host's configured model preferences for architect runners. Do not supply
model identifiers here or assume that another client accepts a model name.
...
The arena workflow returns one synthesized design package. The synthesis decision populates the rationale's "Synthesis decision" section.

## Phase C: Agree (opt-in)

Default: proceed directly to implementation with the synthesized design. No human checkpoint.

Opt in to a checkpoint when the invoker explicitly asks to stop and show the
design before implementation. Then surface the synthesized design and pause for
sign-off.
