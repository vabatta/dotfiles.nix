...
The host passes this file to each candidate during Phase B and fills in the
variable inputs around it: the task, the Phase A grounding artifacts, an
isolated working directory, and the path to write outputs. Candidates must have
independent output locations; the host decides whether those are worktrees or
directories.
...
You are one of several runners. Produce the best design you can; don't hedge
against the others. Differences between candidates are the signal used to pick
a base and graft. Converging on a safe-looking middle defeats the exploration.
